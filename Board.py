...
class Board(object):
    ...
    def isValidCell(self, coord, action):
        ...
        return (0 <= xCoord < NUM_ROWS and 0 <= yCoord < NUM_ROWS)    #TODO : algorithm for checking if cell is valid

    def getCellAfterAction(self, coord, action): # new coord after action
        xCoord, yCoord = coord
        if action == 'left':
            xCoord -= 1
        elif action == 'right':
